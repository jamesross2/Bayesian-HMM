...
class Chain(object):
    ...
    def tabulate(self) -> numpy.array:
        ...
        :return: numpy array with shape (T, 2), where T is the length of the Chain
        """
        return numpy.column_stack(
            [
                [symbol.value for symbol in sequence]
                for sequence in (self.latent_sequence, self.emission_sequence)
            ]
        )
    ...
    def neglogp_chain(
        ...
        p_start = numpy.log(p_initial[self.latent_sequence[0]]) + numpy.log(
            p_emission[self.latent_sequence[0]][self.emission_sequence[0]]
        )
        p_remainder = [
            numpy.log(p_emission[self.latent_sequence[t]][self.emission_sequence[t]])
            + numpy.log(
                p_transition[self.latent_sequence[t - 1]][self.latent_sequence[t]]
            )
