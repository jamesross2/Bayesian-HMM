...
def test_log_likelihood() -> None:
    ...
    stick_breaking_process.add_state(bayesian_hmm.State("A"))
    stick_breaking_process.add_state(bayesian_hmm.State("B"))
    assert stick_breaking_process.log_likelihood() == 0.0

    # when alpha grows, large betas are unlikely
    stick_breaking_process.alpha.value = 3
    stick_breaking_process.value[bayesian_hmm.State("A")] = 0.8
    stick_breaking_process.value[bayesian_hmm.State("B")] = 0.15
    stick_breaking_process.value[bayesian_hmm.AggregateState()] = 0.05
    assert stick_breaking_process.log_likelihood() < 0.0
