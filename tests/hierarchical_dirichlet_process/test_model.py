...
def test_model_initialisation() -> None:
    model = bayesian_hmm.Model()
    assert isinstance(model, bayesian_hmm.Model)
...
def test_model_likelihood() -> None:
    ...
    for val in range(num_symbols):
        model.add_state(bayesian_hmm.Symbol(val))
...
def test_model_resampling() -> None:
    ...
    states = {bayesian_hmm.Symbol(x) for x in {"a", "char", 5}}
    states.add(bayesian_hmm.EmptySymbol())
    counts = {state0: {state1: 3 for state1 in states} for state0 in states}

    for sticky in (True, False):
        model = bayesian_hmm.Model(sticky=sticky)
        _ = [model.add_state(state) for state in states if state != bayesian_hmm.EmptySymbol()]
        model.resample(states=states, counts=counts)
